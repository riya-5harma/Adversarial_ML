...
image = Image.open(image_path).resize((299, 299))  # Resize image to match input size of ResNet-50
x = np.array(image)
...
x_adv = attack.generate(x.reshape((1, 299, 299, 3)))
preds = model.predict(x_adv)
label = decode_predictions(preds, top=3)[0] 
original_label = decode_predictions(model.predict(x.reshape((1, 299, 299, 3))), top=3)[0]
