...
accuracy = np.sum(np.argmax(classifier.predict(x=x_test_adv), axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
...
adv_trainer.fit(x_train, y_train, batch_size=64, nb_epochs=1)

# Evaluate the adversarial model
_, acc = adv_trainer.evaluate(x_test, y_test)
